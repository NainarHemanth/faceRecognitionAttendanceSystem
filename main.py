...
imgBg = cv2.imread('Resources/background.jpg')
...
while True:
    ...
    encodeCurFrame = face_recognition.face_encodings(imgS,
                                                     faceCurFrame)  # previous encoding of known faces and we have new encodings and compare
        ...
        if counter != 0:
            if counter == 1:
                ...
                if secondsElapsed > 30:  # this for 30 sec (in real time use accordingly like 30 min)
                    ...
                    ref.child('Total-attendance').set(studentsInfo[
                                                          'Total-attendance'])  # we need to update total_attendance with the value  studentsInfo['total_attendance']
                    ref.child('Last-attendance-time').set(
                        datetime.now().strftime("%Y-%m-%d %H:%M:%S"))  # updating last attendance time as well
                else:
                    modeType = 3
                    counter = 0
                    imgBg[44:44 + 633, 808:808 + 414] = imgModeList[modeType]
            ...
            if modeType != 3:
                ...
                if counter <= 10:
                    ...
                    cv2.putText(imgBg, str(studentsInfo['Name']), (808 + offset, 445),
                                ...
                                2)  # here name varies big or small length, so we need to center it
...
# to remove the lags use Asynchronous functions
